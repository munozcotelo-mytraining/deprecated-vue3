...
function genText( value : string | number, name : string | undefined,  modifiers : Record<string, boolean> ) : string {
    ...
    if ( name ) {
        output = `MunozDirective - Nombre del argumento : ${ name }, valor del argumento : ${ value }`;
    } else {
        output = `MunozDirective - Sin nombre argumento, valor del argumento : ${ value }`;
    }
...
const munozDirective : vue.Directive = {
